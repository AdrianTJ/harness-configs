...
description: "Write ML papers for NeurIPS/ICML/ICLR: design→submit. Use for starting a paper from an idea or codebase, designing and running experiments, drafting or revising any section, responding to reviews, converting between conference formats, or preparing submission materials."
---
...
Keep a project TODO file (e.g. `paper/TODO.md`) checked into the repo — it is the
persistent state across sessions and survives any context loss:
...
Use the harness's web search tool for broad discovery and its page-fetch tool for
fetching specific papers. The APIs and BibTeX workflow in
[references/citation-workflow.md](references/citation-workflow.md) cover
structured arXiv and Semantic Scholar search programmatically:

```
# Via the web search tool:
"[main technique] + [application domain] site:arxiv.org"
"[baseline method] comparison ICML NeurIPS 2024"

# Via the page-fetch tool (for specific papers):
fetch("https://arxiv.org/abs/2303.17651")
```
...
**Recommended**: If the harness supports MCP servers, add **Exa** for real-time academic search (Claude Code):
```bash
...
**For agent-based workflows**: Delegate each round's queries in parallel to
subagents (Claude Code: `Task` tool; pi: subagents via the `pi-subagents`
package). Collect results, deduplicate, then generate the next round's queries
...
See [references/experiment-patterns.md](references/experiment-patterns.md) for complete design patterns, monitoring, and error recovery.
...
For long-running experiments, set up periodic status checks. The check prompt should follow this template:
...
`references/phase5-paper-drafting.md` — read it when you reach this phase.
...
This skill is written to be harness-neutral, but the full pipeline leans on the
agent's ordinary capabilities — file tools, a terminal, web search, and
subagents — for the research lifecycle. Where a harness differs, notes are
given (Claude Code: `Task` tool subagents; pi: subagents via the
...
| **File tools** | Paper editing, experiment scripts, result files. Use targeted edits for large `.tex` files. |
| **Web search** | Literature discovery: `"[main technique] + [application domain] site:arxiv.org"` |
| **Page fetch** | Fetch paper content, verify citations: `fetch("https://arxiv.org/abs/2303.17651")` |
| **Subagents** | **Parallel section drafting** — spawn isolated subagents for each section. Also for concurrent citation verification. |
...
→ terminal: ls results/
→ terminal or python: analyze results JSON, compute metrics
→ terminal: git add -A && git commit -m '<descriptive message>' && git push
→ report: "Experiment complete: <summary>"
```

**Parallel section drafting** (using subagents):
```
subagent: "Draft the Methods section based on these experiment scripts and configs.
...
subagent: "Draft the Related Work section. Use web search and page fetch to
  find papers. Verify every citation via Semantic Scholar. Group by methodology."

subagent: "Draft the Experiments section. Read all result files in results/.
  State which claim each experiment supports. Include error bars and significance."
```

Each subagent runs as a **fresh context** with no shared history — provide all
necessary information in the prompt. Collect outputs and integrate.

**Citation verification** (run as a Python script):
```python
...
### State Management with Tracked Files

pi and Claude Code have no cross-session memory tool — persistence comes from
files in the repo, which also survive context loss and are shared with the
human author.

**`paper/DECISIONS.md`** — persist key decisions as they are made:
...
Update after major decisions or phase transitions. The TODO list from
[Step 0.5](#step-05-create-a-todo-list) tracks granular progress; the
experiment journal from Step 3.5 tracks the exploration tree.

**Session startup protocol** (on every new session on this project):
```
1. cat paper/TODO.md                   # Check current task list
2. cat paper/DECISIONS.md              # Recall key decisions
3. git log --oneline -10               # Check recent commits
4. ps aux | grep python                # Check running experiments
5. ls results/ | tail -20              # Check for new results
6. Report status to user, ask for direction
```

### Monitoring Long-Running Experiments

pi and Claude Code cannot run unattended scheduled checks. Do this instead:

1. Launch with `nohup ... &` and record the PID (Step 3.1).
2. Rely on the session startup protocol above to check progress each session.
3. If the harness supports scheduled jobs (e.g. Claude Code hooks or a cron
   job that opens a session), keep the check prompt minimal:
...
**[SILENT] protocol**: When nothing has changed since the last check, respond
with exactly `[SILENT]`. Only report when there are genuine changes worth
knowing about.

### Communication Patterns

**When to notify the user** (in your response):
...
Ask the user directly for targeted questions when genuinely blocked:
...
Adapted from `research-paper-writing` v1.1.0 (commit `5598215`, 2026-08-08) in
[NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
(MIT license, authored by Orchestra Research), with the Hermes-specific tool
dialect translated to harness-neutral equivalents for use by pi and Claude
Code. See `references/sources.md` for the upstream history and `SOURCES.md`
