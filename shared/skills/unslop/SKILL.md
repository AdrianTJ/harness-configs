...
7. **Re-anchor after long contexts.** Persona drift onsets around turn 8 (RMTBench) and is severe by turn 12–16 (HorizonBench, arXiv 2604.17283). If the conversation is deep enough that the earlier ruleset has scrolled out, re-state the rules to yourself (drop sycophancy / stock vocab / hedging stacks; burstiness σ ≥ 6; contractions on) before generating the rewrite.
...
Vendored from [MohamedAbdallah-14/unslop](https://github.com/MohamedAbdallah-14/unslop) (MIT),
`skills/unslop/SKILL.md` at commit `5af59d9` (2026-05-05). References to the project's own
CLI and hooks were trimmed — this repo vendors only the skill. See `SOURCES.md` for update
instructions.